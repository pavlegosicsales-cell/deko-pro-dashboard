...
const PUNO = "id, ime, prezime, telefon, proizvod, izvor, info, status, podseti_kad, ishod_beleska, created_at, updated_at, pozvan_kad";
...
export async function citajLeadove(): Promise<{ leadovi: LeadRow[]; error: string | null }> {
  ...
  if (error && /pozvan_kad/.test(error.message)) ({ data, error } = await citaj(PUNO.replace(", pozvan_kad", "")));
